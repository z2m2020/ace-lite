...
def aggregate_metrics(case_results: list[dict[str, Any]]) -> dict[str, float]:
    ...
    reciprocal_ranks = [float(item.get("reciprocal_rank", 0.0)) for item in case_results]
    ...
    repomap_seed_candidates_counts = [
        float(item.get("repomap_seed_candidates_count", 0.0)) for item in case_results
    ]
    repomap_cache_hits = [float(item.get("repomap_cache_hit", 0.0)) for item in case_results]
    ...
    retrieval_context_coverage = [
        float(item.get("retrieval_context_coverage_ratio", 0.0)) for item in case_results
    ]
    retrieval_context_char_counts = [
        float(item.get("retrieval_context_char_count_mean", 0.0)) for item in case_results
    ]
    ...
    chunk_contract_fallback_counts = [
        float(item.get("chunk_contract_fallback_count", 0.0)) for item in case_results
    ]
    chunk_contract_skeleton_chunk_counts = [
        float(item.get("chunk_contract_skeleton_chunk_count", 0.0)) for item in case_results
    ]
    chunk_contract_fallback_ratios = [
        float(item.get("chunk_contract_fallback_ratio", 0.0)) for item in case_results
    ]
    chunk_contract_skeleton_ratios = [
        float(item.get("chunk_contract_skeleton_ratio", 0.0)) for item in case_results
    ]
    chunk_cache_contract_present = [
        float(item.get("chunk_cache_contract_present", 0.0)) for item in case_results
    ]
    chunk_cache_contract_fingerprint_present = [
        float(item.get("chunk_cache_contract_fingerprint_present", 0.0)) for item in case_results
    ]
    chunk_cache_contract_metadata_aligned = [
        float(item.get("chunk_cache_contract_metadata_aligned", 0.0)) for item in case_results
    ]
    chunk_cache_contract_file_counts = [
        float(item.get("chunk_cache_contract_file_count", 0.0)) for item in case_results
    ]
    chunk_cache_contract_chunk_counts = [
        float(item.get("chunk_cache_contract_chunk_count", 0.0)) for item in case_results
    ]
    unsupported_language_fallback_counts = [
        float(item.get("unsupported_language_fallback_count", 0.0)) for item in case_results
    ]
    unsupported_language_fallback_ratios = [
        float(item.get("unsupported_language_fallback_ratio", 0.0)) for item in case_results
    ]
    ...
    robust_signature_coverage = [
        float(item.get("robust_signature_coverage_ratio", 0.0)) for item in case_results
    ]
    ...
    graph_prior_totals = [float(item.get("graph_prior_total", 0.0)) for item in case_results]
    graph_seeded_chunk_counts = [
        float(item.get("graph_seeded_chunk_count", 0.0)) for item in case_results
    ]
    graph_transfer_counts = [float(item.get("graph_transfer_count", 0.0)) for item in case_results]
    graph_hub_suppressed_chunk_counts = [
        float(item.get("graph_hub_suppressed_chunk_count", 0.0)) for item in case_results
    ]
    graph_hub_penalty_totals = [
        float(item.get("graph_hub_penalty_total", 0.0)) for item in case_results
    ]
    graph_closure_enabled = [float(item.get("graph_closure_enabled", 0.0)) for item in case_results]
    graph_closure_boosted_chunk_counts = [
        float(item.get("graph_closure_boosted_chunk_count", 0.0)) for item in case_results
    ]
    ...
    graph_closure_support_edge_counts = [
        float(item.get("graph_closure_support_edge_count", 0.0)) for item in case_results
    ]
    graph_closure_totals = [float(item.get("graph_closure_total", 0.0)) for item in case_results]
    ...
    topological_shield_report_only = [
        float(item.get("topological_shield_report_only", 0.0)) for item in case_results
    ]
    topological_shield_attenuated_chunk_counts = [
        float(item.get("topological_shield_attenuated_chunk_count", 0.0)) for item in case_results
    ]
    topological_shield_coverage = [
        float(item.get("topological_shield_coverage_ratio", 0.0)) for item in case_results
    ]
    topological_shield_attenuation_totals = [
        float(item.get("topological_shield_attenuation_total", 0.0)) for item in case_results
    ]
    graph_source_provider_loaded = [
        float(item.get("graph_source_provider_loaded", 0.0) or 0.0) for item in case_results
    ]
    graph_source_projection_fallback = [
        float(item.get("graph_source_projection_fallback", 0.0) or 0.0) for item in case_results
    ]
    graph_source_edge_counts = [
        float(item.get("graph_source_edge_count", 0.0) or 0.0) for item in case_results
    ]
    ...
    graph_source_centrality_signal_coverage = [
        float(item.get("graph_source_centrality_signal_coverage_ratio", 0.0) or 0.0)
        for item in case_results
    ]
    ...
    chunk_guard_enabled = [float(item.get("chunk_guard_enabled", 0.0)) for item in case_results]
    ...
    chunk_guard_pairwise_conflict_counts = [
        float(item.get("chunk_guard_pairwise_conflict_count", 0.0)) for item in case_results
    ]
    chunk_guard_fallbacks = [float(item.get("chunk_guard_fallback", 0.0)) for item in case_results]
    skills_selected_counts = [
        float(item.get("skills_selected_count", 0.0)) for item in case_results
    ]
    skills_token_budgets = [float(item.get("skills_token_budget", 0.0)) for item in case_results]
    ...
    skills_skipped_for_budget = [
        float(item.get("skills_skipped_for_budget_count", 0.0)) for item in case_results
    ]
    ...
    skills_manifest_metadata_only_selected = [
        float(item.get("skills_manifest_metadata_only_selected_count", 0.0))
        for item in case_results
    ]
    skills_manifest_body_scan_selected = [
        float(item.get("skills_manifest_body_scan_selected_count", 0.0)) for item in case_results
    ]
    skills_selected_matched_count_mean = [
        float(item.get("skills_selected_matched_count_mean", 0.0)) for item in case_results
    ]
    skills_selected_signal_count_mean = [
        float(item.get("skills_selected_signal_count_mean", 0.0)) for item in case_results
    ]
    skills_selected_priority_mean = [
        float(item.get("skills_selected_priority_mean", 0.0)) for item in case_results
    ]
    candidate_rows_materialized_counts = [
        float(item.get("candidate_rows_materialized_count", 0.0) or 0.0) for item in case_results
    ]
    candidate_chunks_materialized_counts = [
        float(item.get("candidate_chunks_materialized_count", 0.0) or 0.0) for item in case_results
    ]
    ...
    skills_markdown_bytes_loaded = [
        float(item.get("skills_markdown_bytes_loaded", 0.0) or 0.0) for item in case_results
    ]
    budget_abort_cases = [float(item.get("budget_abort", 0.0) or 0.0) for item in case_results]
    fallback_taken_cases = [float(item.get("fallback_taken", 0.0) or 0.0) for item in case_results]
    ...
    plan_replay_cache_stale_hit_safe = [
        float(item.get("plan_replay_cache_stale_hit_safe", 0.0)) for item in case_results
    ]
    adaptive_router_enabled_cases = [
        item for item in case_results if float(item.get("router_enabled", 0.0) or 0.0) > 0.0
    ]
    ...
    risk_upgrade_cases = [item for item in elevated_risk_cases if _is_risk_upgrade_case_impl(item)]
    risk_baseline_cases = [
    ...
    validation_probe_enabled = [
        float(item.get("validation_probe_enabled", 0.0) or 0.0) for item in case_results
    ]
    validation_probe_executed_counts = [
        float(item.get("validation_probe_executed_count", 0.0) or 0.0) for item in case_results
    ]
    validation_probe_failed = [
        float(item.get("validation_probe_failed", 0.0) or 0.0) for item in case_results
    ]
    validation_branch_cases = [
        item for item in case_results if float(item.get("validation_branch_case", 0.0) or 0.0) > 0.0
    ]
    ...
    source_plan_validation_feedback_probe_issue_counts = [
        float(item.get("source_plan_validation_feedback_probe_issue_count", 0.0) or 0.0)
        for item in case_results
    ]
    source_plan_validation_feedback_probe_executed_counts = [
        float(item.get("source_plan_validation_feedback_probe_executed_count", 0.0) or 0.0)
        for item in case_results
    ]
    ...
    source_plan_validation_feedback_selected_test_counts = [
        float(item.get("source_plan_validation_feedback_selected_test_count", 0.0) or 0.0)
        for item in case_results
    ]
    source_plan_validation_feedback_executed_test_counts = [
        float(item.get("source_plan_validation_feedback_executed_test_count", 0.0) or 0.0)
        for item in case_results
    ]
    source_plan_failure_signal_present = [
        float(item.get("source_plan_failure_signal_present", 0.0) or 0.0) for item in case_results
    ]
    ...
    source_plan_failure_signal_failed = [
        float(item.get("source_plan_failure_signal_failed", 0.0) or 0.0) for item in case_results
    ]
    source_plan_failure_signal_probe_issue_counts = [
        float(item.get("source_plan_failure_signal_probe_issue_count", 0.0) or 0.0)
        for item in case_results
    ]
    source_plan_failure_signal_probe_executed_counts = [
        float(item.get("source_plan_failure_signal_probe_executed_count", 0.0) or 0.0)
        for item in case_results
    ]
    ...
    source_plan_failure_signal_selected_test_counts = [
        float(item.get("source_plan_failure_signal_selected_test_count", 0.0) or 0.0)
        for item in case_results
    ]
    source_plan_failure_signal_executed_test_counts = [
        float(item.get("source_plan_failure_signal_executed_test_count", 0.0) or 0.0)
        for item in case_results
    ]
    source_plan_failure_signal_replay_cache_origin = [
        1.0
        if str(item.get("source_plan_failure_signal_origin") or "").strip() == "plan_replay_cache"
        else 0.0
        for item in case_results
    ]
    source_plan_failure_signal_observability_origin = [
        1.0
        if str(item.get("source_plan_failure_signal_origin") or "").strip() == "observability"
        else 0.0
        for item in case_results
    ]
    source_plan_failure_signal_source_plan_origin = [
        1.0
        if str(item.get("source_plan_failure_signal_origin") or "").strip() == "source_plan"
        else 0.0
        for item in case_results
    ]
    source_plan_failure_signal_validate_step_origin = [
        1.0
        if str(item.get("source_plan_failure_signal_origin") or "").strip() == "validate_step"
        else 0.0
        for item in case_results
    ]
    source_plan_evidence_card_counts = [
        float(item.get("source_plan_evidence_card_count", 0.0) or 0.0) for item in case_results
    ]
    source_plan_file_card_counts = [
        float(item.get("source_plan_file_card_count", 0.0) or 0.0) for item in case_results
    ]
    source_plan_chunk_card_counts = [
        float(item.get("source_plan_chunk_card_count", 0.0) or 0.0) for item in case_results
    ]
    source_plan_validation_card_present = [
        float(item.get("source_plan_validation_card_present", 0.0) or 0.0) for item in case_results
    ]
    source_plan_direct_ratios = [
        float(item.get("source_plan_direct_evidence_ratio", 0.0) or 0.0) for item in case_results
    ]
    ...
    source_plan_signature_counts = [
        float(item.get("source_plan_signature_count", 0.0) or 0.0) for item in case_results
    ]
    source_plan_skeleton_counts = [
        float(item.get("source_plan_skeleton_count", 0.0) or 0.0) for item in case_results
    ]
    source_plan_robust_signature_counts = [
        float(item.get("source_plan_robust_signature_count", 0.0) or 0.0) for item in case_results
    ]
    ...
    source_plan_signature_ratios = [
        float(item.get("source_plan_signature_ratio", 0.0) or 0.0) for item in case_results
    ]
    source_plan_skeleton_ratios = [
        float(item.get("source_plan_skeleton_ratio", 0.0) or 0.0) for item in case_results
    ]
    source_plan_robust_signature_ratios = [
        float(item.get("source_plan_robust_signature_ratio", 0.0) or 0.0) for item in case_results
    ]
    source_plan_neighbor_context_ratios = [
        float(item.get("source_plan_neighbor_context_ratio", 0.0) or 0.0) for item in case_results
    ]
    source_plan_hint_only_ratios = [
        float(item.get("source_plan_hint_only_ratio", 0.0) or 0.0) for item in case_results
    ]
    ...
    source_plan_packed_path_counts = [
        float(item.get("source_plan_packed_path_count", 0.0) or 0.0) for item in case_results
    ]
    evidence_insufficient = [float(item.get("evidence_insufficient", 0.0)) for item in case_results]
    no_candidate = [float(item.get("evidence_no_candidate", 0.0)) for item in case_results]
    low_support_chunks = [
        float(item.get("evidence_low_support_chunk", 0.0)) for item in case_results
    ]
    missing_validation = [
        float(item.get("evidence_missing_validation", 0.0)) for item in case_results
    ]
    budget_limited = [float(item.get("evidence_budget_limited", 0.0)) for item in case_results]
    noisy_hits = [float(item.get("evidence_noisy_hit", 0.0)) for item in case_results]
    notes_hit_ratios = [float(item.get("notes_hit_ratio", 0.0)) for item in case_results]
    profile_selected_counts = [
        float(item.get("profile_selected_count", 0.0)) for item in case_results
    ]
    ...
    ltm_effective_hit_cases = [
        ...
        if float(item.get("task_success_hit", item.get("utility_hit", 0.0)) or 0.0) > 0.0
    ]
    memory_harmful_negative_control_cases = [
        item
        for item in case_results
        if str(item.get("comparison_lane") or "").strip() == "memory-harmful-negative-control"
    ]
    ...
    ltm_stale_hit_cases = [
        ...
        if float(item.get("ltm_plan_constraint_count", 0.0) or 0.0) > 0.0
        and float(item.get("task_success_hit", item.get("utility_hit", 0.0)) or 0.0) <= 0.0
    ]
    ...
    multi_channel_rrf_enabled = [
        float(item.get("multi_channel_rrf_enabled", 0.0) or 0.0) for item in case_results
    ]
    multi_channel_rrf_applied = [
        float(item.get("multi_channel_rrf_applied", 0.0) or 0.0) for item in case_results
    ]
    multi_channel_rrf_granularity_counts = [
        float(item.get("multi_channel_rrf_granularity_count", 0.0) or 0.0) for item in case_results
    ]
    multi_channel_rrf_pool_sizes = [
        float(item.get("multi_channel_rrf_pool_size", 0.0) or 0.0) for item in case_results
    ]
    ...
    graph_lookup_boosted_counts = [
        float(item.get("graph_lookup_boosted_count", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_weight_scip = [
        float(item.get("graph_lookup_weight_scip", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_weight_xref = [
        float(item.get("graph_lookup_weight_xref", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_weight_query_xref = [
        float(item.get("graph_lookup_weight_query_xref", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_weight_symbol = [
        float(item.get("graph_lookup_weight_symbol", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_weight_import = [
        float(item.get("graph_lookup_weight_import", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_weight_coverage = [
        float(item.get("graph_lookup_weight_coverage", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_candidate_counts = [
        float(item.get("graph_lookup_candidate_count", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_normalizations = [
        str(item.get("graph_lookup_normalization", "") or "").strip() for item in case_results
    ]
    graph_lookup_pool_sizes = [
        float(item.get("graph_lookup_pool_size", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_query_terms_counts = [
        float(item.get("graph_lookup_query_terms_count", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_guard_max_candidates = [
        float(item.get("graph_lookup_guard_max_candidates", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_guard_min_query_terms = [
        float(item.get("graph_lookup_guard_min_query_terms", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_guard_max_query_terms = [
        float(item.get("graph_lookup_guard_max_query_terms", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_query_hit_paths = [
        float(item.get("graph_lookup_query_hit_paths", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_scip_signal_paths = [
        float(item.get("graph_lookup_scip_signal_paths", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_xref_signal_paths = [
        float(item.get("graph_lookup_xref_signal_paths", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_symbol_hit_paths = [
        float(item.get("graph_lookup_symbol_hit_paths", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_import_hit_paths = [
        float(item.get("graph_lookup_import_hit_paths", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_coverage_hit_paths = [
        float(item.get("graph_lookup_coverage_hit_paths", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_max_inbound = [
        float(item.get("graph_lookup_max_inbound", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_max_xref_count = [
        float(item.get("graph_lookup_max_xref_count", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_max_query_hits = [
        float(item.get("graph_lookup_max_query_hits", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_max_symbol_hits = [
        float(item.get("graph_lookup_max_symbol_hits", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_max_import_hits = [
        float(item.get("graph_lookup_max_import_hits", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_max_query_coverage = [
        float(item.get("graph_lookup_max_query_coverage", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_boosted_path_ratios = [
        float(item.get("graph_lookup_boosted_path_ratio", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_query_hit_path_ratios = [
        float(item.get("graph_lookup_query_hit_path_ratio", 0.0) or 0.0) for item in case_results
    ]
    graph_lookup_reasons = [
        str(item.get("graph_lookup_reason", "") or "").strip() for item in case_results
    ]
    native_scip_loaded = [
        float(item.get("native_scip_loaded", 0.0) or 0.0) for item in case_results
    ]
    native_scip_document_counts = [
        float(item.get("native_scip_document_count", 0.0) or 0.0) for item in case_results
    ]
    ...
    native_scip_symbol_definition_counts = [
        float(item.get("native_scip_symbol_definition_count", 0.0) or 0.0) for item in case_results
    ]
    deep_symbol_cases = [
        item for item in case_results if float(item.get("deep_symbol_case", 0.0) or 0.0) > 0.0
    ]
    deep_symbol_recalls = [
        (1.0 if not str(item.get("chunk_stage_miss") or "").strip() else 0.0)
    ...
    embedding_cache_hits = [float(item.get("embedding_cache_hit", 0.0)) for item in case_results]
    embedding_fallbacks = [float(item.get("embedding_fallback", 0.0)) for item in case_results]
    ...
    chunk_semantic_time_budgets = [
        float(item.get("chunk_semantic_time_budget_ms", 0.0)) for item in case_results
    ]
    xref_time_budgets = [float(item.get("xref_time_budget_ms", 0.0)) for item in case_results]
    ...
    embedding_budget_exceeded = [
        float(item.get("embedding_time_budget_exceeded", 0.0)) for item in case_results
    ]
    embedding_adaptive_budget = [
        float(item.get("embedding_adaptive_budget_applied", 0.0)) for item in case_results
    ]
    chunk_semantic_budget_exceeded = [
        float(item.get("chunk_semantic_time_budget_exceeded", 0.0)) for item in case_results
    ]
    chunk_semantic_fallbacks = [
        float(item.get("chunk_semantic_fallback", 0.0)) for item in case_results
    ]
    xref_budget_exhausted = [float(item.get("xref_budget_exhausted", 0.0)) for item in case_results]
    slo_downgrades = [float(item.get("slo_downgrade_triggered", 0.0)) for item in case_results]
    ...
    return {
        ...
        "memory_helpful_task_success_rate": (
            mean(memory_helpful_task_success_hits) if memory_helpful_task_success_hits else 0.0
        ),
        ...
        "multi_channel_rrf_granularity_count_mean": mean(multi_channel_rrf_granularity_counts),
        "multi_channel_rrf_pool_size_mean": mean(multi_channel_rrf_pool_sizes),
        "multi_channel_rrf_granularity_pool_ratio": mean(multi_channel_rrf_granularity_pool_ratios),
        "multi_channel_rrf_granularity_case_ratio": mean(
            [1.0 if value > 0.0 else 0.0 for value in multi_channel_rrf_granularity_counts]
        ),
        ...
        "graph_lookup_guard_max_candidates_mean": mean(graph_lookup_guard_max_candidates),
        "graph_lookup_guard_min_query_terms_mean": mean(graph_lookup_guard_min_query_terms),
        "graph_lookup_guard_max_query_terms_mean": mean(graph_lookup_guard_max_query_terms),
        ...
        "graph_lookup_coverage_hit_paths_mean": mean(graph_lookup_coverage_hit_paths),
        ...
        "graph_lookup_max_query_coverage_mean": mean(graph_lookup_max_query_coverage),
        ...
        "deep_symbol_case_recall": (mean(deep_symbol_recalls) if deep_symbol_recalls else 0.0),
        ...
        "native_scip_symbol_definition_count_mean": mean(native_scip_symbol_definition_counts),
        ...
        "repomap_seed_candidates_count_mean": mean(repomap_seed_candidates_counts),
        ...
        "chunk_contract_fallback_count_mean": mean(chunk_contract_fallback_counts),
        "chunk_contract_skeleton_chunk_count_mean": mean(chunk_contract_skeleton_chunk_counts),
        ...
        "chunk_cache_contract_present_ratio": mean(chunk_cache_contract_present),
        "chunk_cache_contract_fingerprint_present_ratio": mean(
            chunk_cache_contract_fingerprint_present
        ),
        "chunk_cache_contract_metadata_aligned_ratio": mean(chunk_cache_contract_metadata_aligned),
        "chunk_cache_contract_file_count_mean": mean(chunk_cache_contract_file_counts),
        "chunk_cache_contract_chunk_count_mean": mean(chunk_cache_contract_chunk_counts),
        "unsupported_language_fallback_count_mean": mean(unsupported_language_fallback_counts),
        "unsupported_language_fallback_ratio": mean(unsupported_language_fallback_ratios),
        ...
        "graph_source_pagerank_signal_coverage_ratio": mean(graph_source_pagerank_signal_coverage),
        ...
        "chunk_guard_pairwise_conflict_count_mean": mean(chunk_guard_pairwise_conflict_counts),
        ...
        "skills_manifest_metadata_only_selected_mean": mean(skills_manifest_metadata_only_selected),
        "skills_manifest_body_scan_selected_mean": mean(skills_manifest_body_scan_selected),
        "skills_selected_matched_count_mean": mean(skills_selected_matched_count_mean),
        "skills_selected_signal_count_mean": mean(skills_selected_signal_count_mean),
        "skills_selected_priority_mean": mean(skills_selected_priority_mean),
        "candidate_rows_materialized_mean": mean(candidate_rows_materialized_counts),
        "candidate_chunks_materialized_mean": mean(candidate_chunks_materialized_counts),
        ...
        "plan_replay_cache_stale_hit_safe_ratio": mean(plan_replay_cache_stale_hit_safe),
        "adaptive_router_shadow_coverage": (
            float(len(adaptive_router_shadow_cases)) / float(len(adaptive_router_enabled_cases))
        ...
        "risk_upgrade_precision_gain": (
            ...
            - mean(float(item.get("precision_at_k", 0.0) or 0.0) for item in risk_baseline_cases)
        ...
        "validation_probe_executed_count_mean": mean(validation_probe_executed_counts),
        ...
        "validation_branch_candidate_count_mean": (
            mean(validation_branch_candidate_counts) if validation_branch_candidate_counts else 0.0
        ),
        "validation_branch_rejected_count_mean": (
            mean(validation_branch_rejected_counts) if validation_branch_rejected_counts else 0.0
        ),
        ...
        "validation_branch_archive_present_ratio": (
            mean(validation_branch_archive_present) if validation_branch_archive_present else 0.0
        ),
        ...
        "validation_branch_winner_pass_rate": (
            mean(validation_branch_winner_passed) if validation_branch_winner_passed else 0.0
        ),
        "validation_branch_winner_regressed_rate": (
            mean(validation_branch_winner_regressed) if validation_branch_winner_regressed else 0.0
        ),
        "validation_branch_winner_score_mean": (
            mean(validation_branch_winner_scores) if validation_branch_winner_scores else 0.0
        ),
        ...
        "source_plan_evidence_card_count_mean": mean(source_plan_evidence_card_counts),
        ...
        "source_plan_validation_card_present_ratio": mean(source_plan_validation_card_present),
        ...
        "source_plan_failure_signal_present_ratio": mean(source_plan_failure_signal_present),
        "source_plan_failure_signal_issue_count_mean": mean(
            source_plan_failure_signal_issue_counts
        ),
        "source_plan_failure_signal_failure_rate": mean(source_plan_failure_signal_failed),
        ...
        "source_plan_robust_signature_count_mean": mean(source_plan_robust_signature_counts),
        ...
        "source_plan_robust_signature_ratio": mean(source_plan_robust_signature_ratios),
        "source_plan_neighbor_context_ratio": mean(source_plan_neighbor_context_ratios),
        ...
        "ltm_false_help_rate": (
            float(len(ltm_false_help_cases)) / float(len(memory_harmful_negative_control_cases))
        ...
        "ltm_replay_drift_rate": (
            float(len(ltm_replay_drift_cases)) / float(len(ltm_time_sensitive_hit_cases))
        ...
        "issue_report_linked_plan_rate": (
            float(len(issue_report_linked_plan_cases)) / float(len(issue_report_linked_cases))
            if issue_report_linked_cases
            else 0.0
        ),
        "issue_to_benchmark_case_conversion_rate": (
            float(len(issue_report_linked_cases)) / float(len(issue_report_feedback_cases))
        ...
        "dev_feedback_resolution_rate": (
            mean(dev_feedback_resolution_hits) if dev_feedback_resolution_hits else 0.0
        ),
        ...
        "chunk_semantic_time_budget_exceeded_ratio": mean(chunk_semantic_budget_exceeded),
...
def build_retrieval_control_plane_gate_summary(
    *,
    ...
    benchmark_regression_passed = regression_evaluated and not benchmark_regression_detected
    shadow_coverage_passed = adaptive_router_shadow_coverage >= shadow_coverage_threshold
    ...
    return {
        ...
        "risk_upgrade_precision_gain_threshold": (risk_upgrade_precision_gain_threshold),
...
def build_retrieval_frontier_gate_summary(
    *,
    ...
    normalized_metrics = normalize_metrics(metrics)
    deep_symbol_case_recall = float(normalized_metrics.get("deep_symbol_case_recall", 0.0) or 0.0)
    native_scip_loaded_rate = float(normalized_metrics.get("native_scip_loaded_rate", 0.0) or 0.0)
    ...
    deep_symbol_case_recall_passed = deep_symbol_case_recall >= deep_symbol_case_recall_threshold
    native_scip_loaded_rate_passed = native_scip_loaded_rate >= native_scip_loaded_rate_threshold
...
def build_repomap_seed_summary(*, metrics: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "seed_candidates_count_mean": round(
            float(normalized_metrics.get("repomap_seed_candidates_count_mean", 0.0) or 0.0),
            6,
        ),
...
def build_native_scip_summary(*, metrics: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "document_count_mean": round(
            float(normalized_metrics.get("native_scip_document_count_mean", 0.0) or 0.0),
            6,
        ),
        "definition_occurrence_count_mean": round(
            float(
                normalized_metrics.get("native_scip_definition_occurrence_count_mean", 0.0) or 0.0
            ),
            6,
        ),
        "reference_occurrence_count_mean": round(
            float(
                normalized_metrics.get("native_scip_reference_occurrence_count_mean", 0.0) or 0.0
            ),
            6,
        ),
        "symbol_definition_count_mean": round(
            float(normalized_metrics.get("native_scip_symbol_definition_count_mean", 0.0) or 0.0),
            6,
        ),
    }
...
def build_validation_probe_summary(*, metrics: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "probe_enabled_ratio": round(
            float(normalized_metrics.get("validation_probe_enabled_ratio", 0.0) or 0.0),
            6,
        ),
        "probe_executed_count_mean": round(
            float(normalized_metrics.get("validation_probe_executed_count_mean", 0.0) or 0.0),
            6,
        ),
        "probe_failure_rate": round(
            float(normalized_metrics.get("validation_probe_failure_rate", 0.0) or 0.0),
            6,
        ),
    }
...
def build_validation_branch_summary(*, metrics: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "candidate_count_mean": round(
            float(normalized_metrics.get("validation_branch_candidate_count_mean", 0.0) or 0.0),
            6,
        ),
        "rejected_count_mean": round(
            float(normalized_metrics.get("validation_branch_rejected_count_mean", 0.0) or 0.0),
            6,
        ),
        "selection_present_ratio": round(
            float(normalized_metrics.get("validation_branch_selection_present_ratio", 0.0) or 0.0),
            6,
        ),
        "patch_artifact_present_ratio": round(
            float(
                normalized_metrics.get("validation_branch_patch_artifact_present_ratio", 0.0) or 0.0
            ),
            6,
        ),
        "archive_present_ratio": round(
            float(normalized_metrics.get("validation_branch_archive_present_ratio", 0.0) or 0.0),
            6,
        ),
        "parallel_case_rate": round(
            float(normalized_metrics.get("validation_branch_parallel_case_rate", 0.0) or 0.0),
            6,
        ),
        "winner_pass_rate": round(
            float(normalized_metrics.get("validation_branch_winner_pass_rate", 0.0) or 0.0),
            6,
        ),
        "winner_regressed_rate": round(
            float(normalized_metrics.get("validation_branch_winner_regressed_rate", 0.0) or 0.0),
            6,
        ),
        "winner_score_mean": round(
            float(normalized_metrics.get("validation_branch_winner_score_mean", 0.0) or 0.0),
            6,
        ),
        "winner_after_issue_count_mean": round(
            float(
                normalized_metrics.get("validation_branch_winner_after_issue_count_mean", 0.0)
                or 0.0
            ),
            6,
        ),
...
def build_validation_branch_gate_summary(*, metrics: dict[str, Any]) -> dict[str, Any]:
    normalized_metrics = normalize_metrics(metrics)
    ...
    selection_present_ratio = float(
        normalized_metrics.get("validation_branch_selection_present_ratio", 0.0) or 0.0
    )
    ...
    selection_present_ratio_passed = selection_present_ratio >= selection_present_ratio_threshold
    patch_artifact_present_ratio_passed = (
        patch_artifact_present_ratio >= patch_artifact_present_ratio_threshold
    )
    archive_present_ratio_passed = archive_present_ratio >= archive_present_ratio_threshold
    ...
    return {
        ...
        "patch_artifact_present_ratio_threshold": (patch_artifact_present_ratio_threshold),
        "patch_artifact_present_ratio_passed": (patch_artifact_present_ratio_passed),
...
def build_source_plan_card_summary(*, metrics: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "validation_card_present_ratio": round(
            float(normalized_metrics.get("source_plan_validation_card_present_ratio", 0.0) or 0.0),
            6,
        ),
    }


def build_source_plan_validation_feedback_summary(*, metrics: dict[str, Any]) -> dict[str, float]:
    normalized_metrics = normalize_metrics(metrics)
    return {
        "present_ratio": round(
            float(
                normalized_metrics.get("source_plan_validation_feedback_present_ratio", 0.0) or 0.0
            ),
            6,
        ),
        "issue_count_mean": round(
            float(
                normalized_metrics.get("source_plan_validation_feedback_issue_count_mean", 0.0)
                or 0.0
            ),
            6,
        ),
        "failure_rate": round(
            float(
                normalized_metrics.get("source_plan_validation_feedback_failure_rate", 0.0) or 0.0
            ),
            6,
        ),
        ...
        "probe_failure_rate": round(
            float(
                normalized_metrics.get("source_plan_validation_feedback_probe_failure_rate", 0.0)
                or 0.0
            ),
            6,
        ),
...
def build_source_plan_failure_signal_summary(*, metrics: dict[str, Any]) -> dict[str, float]:
    normalized_metrics = normalize_metrics(metrics)
    return {
        "present_ratio": round(
            float(normalized_metrics.get("source_plan_failure_signal_present_ratio", 0.0) or 0.0),
            6,
        ),
        "issue_count_mean": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_issue_count_mean", 0.0) or 0.0
            ),
            6,
        ),
        "failure_rate": round(
            float(normalized_metrics.get("source_plan_failure_signal_failure_rate", 0.0) or 0.0),
            6,
        ),
        "probe_issue_count_mean": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_probe_issue_count_mean", 0.0)
                or 0.0
            ),
            6,
        ),
        "probe_executed_count_mean": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_probe_executed_count_mean", 0.0)
                or 0.0
            ),
            6,
        ),
        "probe_failure_rate": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_probe_failure_rate", 0.0) or 0.0
            ),
            6,
        ),
        "selected_test_count_mean": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_selected_test_count_mean", 0.0)
                or 0.0
            ),
            6,
        ),
        "executed_test_count_mean": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_executed_test_count_mean", 0.0)
                or 0.0
            ),
            6,
        ),
        "replay_cache_origin_ratio": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_replay_cache_origin_ratio", 0.0)
                or 0.0
            ),
            6,
        ),
        "observability_origin_ratio": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_observability_origin_ratio", 0.0)
                or 0.0
            ),
            6,
        ),
        "source_plan_origin_ratio": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_source_plan_origin_ratio", 0.0)
                or 0.0
            ),
            6,
        ),
        "validate_step_origin_ratio": round(
            float(
                normalized_metrics.get("source_plan_failure_signal_validate_step_origin_ratio", 0.0)
                or 0.0
            ),
            6,
        ),
...
def build_evidence_insufficiency_summary(
    ...
    return cast(dict[str, Any], _build_evidence_insufficiency_summary_impl(case_results))


def build_missing_context_risk_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_missing_context_risk_summary_impl(case_results))


def build_chunk_stage_miss_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_chunk_stage_miss_summary_impl(case_results))


def build_decision_observability_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_decision_observability_summary_impl(case_results))
...
def build_retrieval_default_strategy_summary(
    ...
    return cast(dict[str, Any], _build_retrieval_default_strategy_summary_impl(case_results))


def build_agent_loop_control_plane_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_agent_loop_control_plane_summary_impl(case_results))


def build_preference_observability_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_preference_observability_summary_impl(case_results))


def build_feedback_observability_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_feedback_observability_summary_impl(case_results))


def build_feedback_loop_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_feedback_loop_summary_impl(case_results))


def build_ltm_explainability_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_ltm_explainability_summary_impl(case_results))


def build_adaptive_router_arm_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_adaptive_router_arm_summary_impl(case_results))


def build_learning_router_rollout_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_learning_router_rollout_summary_impl(case_results))


def build_adaptive_router_pair_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_adaptive_router_pair_summary_impl(case_results))


def build_adaptive_router_observability_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_adaptive_router_observability_summary_impl(case_results))


def build_comparison_lane_summary(case_results: list[dict[str, Any]]) -> dict[str, Any]:
    return cast(dict[str, Any], _build_comparison_lane_summary_impl(case_results))


def build_stage_latency_summary(case_results: list[dict[str, Any]]) -> dict[str, Any]:
    return cast(dict[str, Any], _build_stage_latency_summary_impl(case_results))


def build_slo_budget_summary(case_results: list[dict[str, Any]]) -> dict[str, Any]:
    return cast(dict[str, Any], _build_slo_budget_summary_impl(case_results))


def build_wave1_context_governance_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_wave1_context_governance_summary_impl(case_results))


def build_context_refine_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_context_refine_summary_impl(case_results))


def build_chunk_cache_contract_summary(
    case_results: list[dict[str, Any]],
) -> dict[str, Any]:
    return cast(dict[str, Any], _build_chunk_cache_contract_summary_impl(case_results))
