...
error_keywords: []
default_sections: [Workflow, Borrowing Matrix, Prompt Template, Iteration Loop, Output Contract]
...
4. Build a borrowing matrix before editing code. Force each candidate idea through fit, cost, and rollback checks.
5. Borrow the smallest high-value idea first. Prefer helper extraction, contract hardening, naming alignment, report/readability improvements, or narrow regression guards over broad rewrites.
6. Preserve what should remain project-specific. Do not copy source-repo abstractions, tooling, or automation just because they exist.
7. Land one minimal improvement, validate it, then record accepted ideas, rejected ideas, and the next candidate.
...
3. After validation, record:
   - accepted borrowing
   - rejected or deferred borrowings
   - what additional evidence would change the next decision
...
- Validation evidence
- Explicit rejected or deferred ideas
